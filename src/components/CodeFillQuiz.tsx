...
export function CodeFillQuiz({
  ...
  // Split code template into parts around ___ markers
  const codeParts = useMemo(() => {
    if (!quiz.codeTemplate) return [];
    return quiz.codeTemplate.split("___");
  }, [quiz.codeTemplate]);

  const langLabel =
    LANGUAGE_LABELS[quiz.codeLanguage ?? ""] ?? quiz.codeLanguage ?? "";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {langLabel && (
          <span className="text-xs text-gray-500 ml-auto">{langLabel}</span>
        )}
      </div>
      ...
      <div className="border border-gray-700 rounded-lg overflow-hidden">
        <pre className="p-4 bg-gray-900 text-sm text-gray-300 overflow-x-auto font-mono leading-relaxed whitespace-pre-wrap">
            ...
            <span key={i}>
              ...
              {i < codeParts.length - 1 && (
                <span className="inline-block align-baseline">
                  {isAnswered ? (
                    <span
                      className={`inline-block px-2 py-0.5 rounded text-sm font-mono border ${
                    ...
                    <button
                      ...
                      className={`inline-block min-w-[60px] px-2 py-0.5 rounded text-sm font-mono transition-all ${
                        filledValues[i] !== null
                    ...
                    <input
                      ...
                      placeholder={`빈칸 ${i + 1}`}
                      className="inline-block w-32 px-2 py-0.5 rounded border border-blue-500/50 bg-gray-800 text-blue-300 text-sm font-mono placeholder-gray-600 focus:outline-none focus:border-blue-400"
                    />
                  )}
                </span>
              )}
