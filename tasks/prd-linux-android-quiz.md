# PRD: Linux Kernel & Android System 학습 퀴즈 웹앱

## Introduction

Linux Kernel과 Android System에 대한 지식을 테스트하고 학습할 수 있는 한국어 퀴즈 웹앱이다. 사용자는 두 카테고리 중 하나를 선택하고, 서브 카테고리와 난이도(초급/중급/고급/랜덤)를 설정하여 선다형·주관식·코드 빈칸 채우기 등 다양한 형태의 문제를 풀 수 있다. 500개 이상의 퀴즈를 정적 데이터로 포함하며, 로그인 없이 로컬 스토리지로 진행 상황을 관리한다. UI는 한국어, 코드와 기술 용어는 영어 원문 그대로 사용한다.
